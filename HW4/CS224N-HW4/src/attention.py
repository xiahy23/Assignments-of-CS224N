...
def precompute_rotary_emb(dim, max_positions):
    ...
    '''rope_cache = torch.stack([
        torch.stack([
            torch.cos(torch.arange(max_positions) * (1/10000)**(-2*(i-1)/dim)),
            torch.sin(torch.arange(max_positions) * (1/10000)**(-2*(i-1)/dim))
        ], dim=-1) for i in range(1, int(dim/2) + 1)
    ], dim=1)'''
    rope_cache = torch.stack([
        torch.stack([
            torch.exp(1j*(torch.arange(1, max_positions+1) * (1/10000)**(-2*(i-1)/dim)))
        ], dim=-1) for i in range(1, dim//2 + 1)
    ], dim=1)
    rope_cache = rope_cache.squeeze()
    ### END YOUR CODE ###
    return rope_cache
...
def apply_rotary_emb(x, rope_cache):
    ...
    ### YOUR CODE HERE ###
    seq_len = x.size(-2)
    rope_cache_trunc = rope_cache[:seq_len]
    #x_reshape = torch.stack(torch.split(x, 2, dim=-1), dim=-2)
    x_reshape = x.view(x.shape[0], x.shape[1], x.shape[2], x.shape[3]//2, 2)
    x_complex = torch.view_as_complex(x_reshape)
    rope_complex = rope_cache_trunc#torch.view_as_complex(rope_cache_trunc)
    rotated_x_complex = x_complex * rope_complex
    rotated_x_real = torch.view_as_real(rotated_x_complex)
    #rotated_x = torch.cat(torch.unbind(rotated_x_real, dim=-2), dim=-1)
    rotated_x = rotated_x_real.view(rotated_x_real.shape[0], rotated_x_real.shape[1], rotated_x_real.shape[2], rotated_x_real.shape[3] * 2)
    ### END YOUR CODE ###
    return rotated_x
