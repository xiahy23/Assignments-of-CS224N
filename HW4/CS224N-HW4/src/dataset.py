...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        mask_index = random.randint(0, trunc_length - mask_length)
        
        prefix = truncated_doc[:mask_index]
        mask = truncated_doc[mask_index:mask_index+mask_length]
        suffix = truncated_doc[mask_index+mask_length:]
        
        masked_string = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + mask
        masked_string += self.PAD_CHAR * (self.block_size - len(masked_string) + 1) #"+1" serve the following process
        x = masked_string[:-1]
        y = masked_string[1:]
